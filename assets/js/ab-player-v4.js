...
function initializePlayers(players) {
  players.forEach((player) => {
    ...
    const playSound = (sound) => {
      ...
      stopButton.disabled = false; // Enable stop button on play
    };
    ...
    playButton.addEventListener('click', () => {
      if (currentSound) {
        ...
        selectTrack(soundA, aButton); // Default to A if nothing selected
      }
    });
    ...
    // Initial state setup
    playButton.innerHTML = playIcon; // Set initial play icon
    aButton.disabled = false;
    bButton.disabled = false;
    if (cButton) cButton.disabled = false;
    ...
    if (!soundA.src || !soundB.src || (soundC && !soundC.src)) {
      ...
      aButton.disabled = true; // Start with A selected (but not playing)
      currentSound = soundA;
    }
  });
}